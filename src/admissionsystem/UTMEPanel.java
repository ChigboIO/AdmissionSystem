 ...
 * @author SCIENCE
 */
public class UTMEPanel extends JPanel {
    ...
    public UTMEPanel(String dept){
        ...
        required_grid_panel.setLayout(new GridLayout(4, 2, 5,5));
        for(int i = 0; i < required.length; i++){
        ...
        required_panel.setBorder(new EmptyBorder(5,5,200,5));
        ...
        optional_panel.setBorder(new EmptyBorder(5,5,50,5));
        
        save = new JButton("Save UTME Subjects");
        ...
        pane.setText("A student applying for admission into "+ dept +" must take the following subjects in UTME / PUTME:");
    ...
    private void selectSaved(){
        try{
            ...
            result = statement.executeQuery("SELECT * FROM utme_optional WHERE department = '"+ dept +"'");
            result.beforeFirst();
            i = 0;
            while(result.next()){
            ...
            if(result.first())
                optionals.setText(String.valueOf(result.getInt("utme_number_optional")));
            */
        } catch(SQLException ex){
            JOptionPane.showMessageDialog(null, "SQL error occoured");
        }
    ...
    public void doSave(){
        ...
        if(repeated){
            ...
            if(count < num){
                ...
                try{
                    //statement.execute("UPDATE departments SET utme_number_optional = "+ num +" WHERE departmentName = '"+ dept +"'");

                    statement.execute("DELETE FROM utme_required WHERE department = '"+ dept +"'");
                    statement.execute("DELETE FROM utme_optional WHERE department = '"+ dept +"'");

                    for (JComboBox required1 : required) {
                        if(required1.getSelectedIndex() != 0){
                            statement.execute("INSERT INTO utme_required(department, subject) VALUES('"+ dept +"', '"+ required1.getSelectedItem().toString()
                                    +"')");
                        }
                    }
                    ...
                    for (JComboBox optional1 : optional) {
                        if(optional1.getSelectedIndex() != 0){
                            statement.execute("INSERT INTO utme_optional(department, subject) VALUES('"+ dept +"', '"+ optional1.getSelectedItem().toString()
                                    +"')");
                        }
                    }

                    JOptionPane.showMessageDialog(null, "UTME Subjects saved!!!");
